...
export default function ThemePickerDialog({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-[var(--blur-sm)] transition-opacity duration-[var(--t-quick)] ease-[var(--ease-out)] data-[starting-style]:opacity-0 data-[ending-style]:opacity-0 motion-reduce:transition-none" />
        <Dialog.Viewport className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto p-4">
          <Dialog.Popup
            finalFocus={triggerRef ?? true}
            className="w-full max-w-5xl rounded-[32px] border border-[var(--glass-border)] bg-[var(--glass-1)] p-5 text-[var(--color-text)] shadow-[var(--shadow-lg)] backdrop-blur-[var(--blur-lg)] transition-[opacity,transform] duration-[var(--t-base)] ease-[var(--ease-out)] data-[starting-style]:scale-95 data-[starting-style]:opacity-0 data-[ending-style]:scale-95 data-[ending-style]:opacity-0 motion-reduce:transition-none motion-reduce:data-[starting-style]:scale-100 motion-reduce:data-[ending-style]:scale-100 sm:p-6"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <Dialog.Title className="font-display text-3xl text-[var(--color-text)] sm:text-4xl">
                  Escolha um Tema
                </Dialog.Title>
              ...
              <Dialog.Close
                className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[var(--glass-border)] bg-[var(--glass-1)] text-[var(--color-text)] transition-[opacity,background-color] duration-[var(--t-quick)] ease-[var(--ease-out)] hover:bg-[var(--glass-2)] focus-visible:outline-none focus-visible:shadow-[var(--ring-focus)]"
